...
pub trait SubscriptionRepository: Send + Sync + 'static {
    ...
    async fn find_by_domain(&self, user_id: &UserId, domain: &str) -> Result<Subscription, DomainError>;
    ...
    async fn delete_one(&self, user_id: &UserId, domain: &str) -> Result<(), DomainError>;
} 